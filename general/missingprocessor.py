        ...
        if self.missing:
            ...
            if self.dtype == object:
                ...
                if isinstance(self.model, LabelBinarizer):
                    filldata = np.array(
                        [str(self.non) if x != x else str(x) for x in data])
                else:
                    filldata = np.array(
                        [self.non if x != x else x for x in data])
            else:
                loc = np.isnan(data)
        ...
        for i, (types, col) in enumerate(zip(self.types, data.columns)):
            ...
            if types == 'continuous':
                if all([float(x).is_integer() for x in data[col].unique() if float(x) == float(x)]):
                    ...
                    types = 'int'
            model = MissingProcessor(
                which=types, name=col, use_pri=self.use_pri)
            if col == self.use_pri:
                tmp = value.tolist()
                tmp.append(0)
                value = np.array(tmp)
            ...
            print(model.name, model.tgt_len,
                  model.threshold, model.which, model.non)
