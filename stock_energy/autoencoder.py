...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, embed_dim, layers, dropout):
        ...
        self.layers = layers
        self.rnn = nn.GRU(input_dim, hidden_dim, layers,
...
class Decoder(nn.Module):
    def __init__(self, processors, hidden_dim, statics_dim, dynamics_dim, layers, dropout):
        ...
        self.layers = layers
        self.rnn = nn.GRU(hidden_dim + dynamics_dim, hidden_dim,
                          layers, batch_first=True, dropout=dropout)
    ...
    def forward(self, embed, dynamics, seq_len, forcing=0.5):
        ...
        for i in range(max_len):
            ...
            out = apply_activation(
                self.d_P, self.dynamics_fc(out).squeeze(1)).unsqueeze(1)
    ...
    def generate_dynamics(self, embed, max_len):
        ...
        for i in range(max_len):
            ...
            out, hidden = self.rnn(x, hidden)
            out = apply_activation(
                self.d_P, self.dynamics_fc(out).squeeze(1)).detach()
            # out = torch.FloatTensor(self.d_P.transform(self.d_P.inverse_transform(out.cpu().numpy()))).to(embed.device)
            x = out.unsqueeze(1)
            res.append(x)
        res = torch.cat(res, dim=1)
...
class Autoencoder(nn.Module):
    def __init__(self, processors, hidden_dim, embed_dim, layers, dropout=0.0):
        ...
        self.encoder = Encoder(statics_dim + dynamics_dim,
                               hidden_dim, embed_dim, layers, dropout)
        self.decoder = Decoder(processors, hidden_dim,
                               statics_dim, dynamics_dim, layers, dropout)
...
class VariationalAutoencoder(nn.Module):
    def __init__(self, processors, hidden_dim, embed_dim, layers, dropout=0.0):
        ...
        self.encoder = VariationalEncoder(
            statics_dim + dynamics_dim, hidden_dim, embed_dim, layers, dropout)
        self.decoder = Decoder(processors, hidden_dim,
                               statics_dim, dynamics_dim, layers, dropout)
    ...
    def forward(self, statics, dynamics, seq_len):
        mu, logvar = self.encoder(statics, dynamics, seq_len)
