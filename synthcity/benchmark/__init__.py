...
class Benchmarks:
    ...
    def evaluate(
        # test name, plugin name, plugin args
        ...
        # classification, regression, survival_analysis, time_series
        ...
        **generate_kwargs: Any,
        ...
        for testcase, plugin, kwargs in tests:
            ...
            if not isinstance(kwargs, dict):
                raise ValueError(
                    f"'kwargs' must be a dict for {testcase}:{plugin}")
            ...
            augmentation_hash_object = hashlib.sha256(
                augmentation_arguments_hash_raw)
            ...
            for repeat in repeats_list:
                ...
                if X_syn_cache_file.exists() and synthetic_reuse_if_exists:
                    ...
                    try:
                        ...
                        if len(X_syn) == 0:
                            raise RuntimeError(
                                "Plugin failed to generate data")
                ...
                if metrics and any(
                    ...
                    if augment_generator_file.exists() and augmented_reuse_if_exists:
                        augment_generator = load_from_file(
                            augment_generator_file)
                    else:
                        ...
                        try:
                            ...
                            continue
                        if synthetic_cache:
                            save_to_file(augment_generator_file,
                                         augment_generator)
                ...
                if 'target' in X_syn.columns:
                    X_syn.data['target'] = X_syn.data['target'].astype(
                        np.float32)
