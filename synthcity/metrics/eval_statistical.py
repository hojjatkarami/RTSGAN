...
class JensenShannonDistance(StatisticalEvaluator):
    ...
    def _evaluate_stats(
        ...
        for col in X_gt.columns:
            ...
            X_gt_bin, gt_bins = pd.cut(
                X_gt[col], bins=local_bins, retbins=True)
...
class AlphaPrecision(StatisticalEvaluator):
    ...
    def metrics(
        ...
        if Delta_precision_alpha < 0:
            raise RuntimeError(
                "negative value detected for Delta_precision_alpha")
        ...
        if Delta_coverage_beta < 0:
            raise RuntimeError(
                "negative value detected for Delta_coverage_beta")
    ...
    def _evaluate(
        ...
        results[f"alpha_precision_curve{emb}"] = alpha_precision_curve
        results[f"beta_coverage_curve{emb}"] = beta_coverage_curve
        ...
        results["alpha_precision_curve_naive"] = alpha_precision_curve_naive
        results["beta_coverage_curve_naive"] = beta_coverage_curve_naive
